process frag4 {
    ...
    singularity exec docker://staphb/bwa:0.7.17 bwa index ${params.reference}/NC_002640.1_DENV4.fasta
    singularity exec docker://staphb/bwa:0.7.17 bwa mem ${params.reference}/NC_002640.1_DENV4.fasta ${mypath}/\${samplename}_1.fq.gz ${mypath}/\${samplename}_2.fq.gz | singularity exec docker://staphb/samtools:1.12 samtools view - -F 4 -u -h | singularity exec docker://staphb/samtools:1.12 samtools sort -n > ${mypath}/alignment/\${samplename}.namesorted.bam
