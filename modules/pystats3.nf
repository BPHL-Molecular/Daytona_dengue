process pystats3 {
    ...
    if qc_flag == 'PASS':
        ...
        subprocess.run('cp ' + "${mypath}" + '/variants/' + items[-1] + '.variants.tsv ' + "${params.output}"+'/dengue3/variants/', shell=True, check=True)
        ...
        if vadr_flag == 'REVIEW':
            subprocess.run("cp " + "${mypath}"+"/assembly/vadr_results/vadr_results.vadr.alt.list "+"${params.output}"+"/dengue3/vadr_error_reports/"+items[-1]+".vadr.alt.list", shell=True, check=True)
